...
var createComment = function (values) {
    var toCreate = {};
    values = values || {};
    // set required properties
    toCreate.body = values.body || "body-value";
    return createInstance('blog.Comment', toCreate);
};
...
suite('Blog CRUD tests', function() {
    ...
    suite('Comment', function() {
        var entity;
        test('GET entity', function(done) {
            getExactEntity('blog.Comment').then(function(fetched) {
                entity = fetched; 
                assert.equal(fetched.fullName, "blog.Comment");
                assert.ok(fetched.extentUri);
                assert.ok(fetched.templateUri);
            }).then(done, done);
        });
        ...
        test('POST', function(done) {
            createComment().then(function(created) {
                assert.ok(created);
                assert.ok(created.uri);
            }).then(done, done);
        });
        test('GET one', function(done) {
            var created;
            createComment().then(function(result) {
                created = result;
                return httpClient.performRequestOnURL(created.uri, null, 200);
            }).then(function(retrieved) {
                assert.ok(retrieved);
